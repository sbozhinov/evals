$(document).ready(function () {
    ...
    $("#ModButton").click(function () {
        ...
        for (var count = list.options.length - 1; count >= 0; count--) {
            ...
            if (list.options[count].selected == true) {
                valid = true;
                courseUpdate = list.options[count].text;
                break;
            }
        }
        if (valid) {
            newInput = window.prompt("Enter new instructor: ");
            var array = {course: courseUpdate, instructor: newInput};
            $.ajax({
                url: 'instructors/modifyInstructor',
                data: array,
                type: 'POST',
                success: function () {
            ...
            alert("You must select a class to modify.");
    ...
    $('#courseNumber').change(function () {
        ...
        if (data == "*") {
            ...
            var data = {
                courseNum: courseNumber.val(),
                instructor: "*"
            };
            //ajax call to the controller
            $.ajax({
                url: 'reports/queryCourseNumber',
                data: data,
                type: 'POST',
                dataType: 'JSON',
                success: function (data) {
